...
def normalize_plate_text(raw: str) -> str | None:
    """Validate and clean an OCR string into a canonical Indian plate.

    Accepts the standard grammar ``^[A-Z]{2}[0-9]{2}[A-Z]{0,2}[0-9]{4}$``.
    Because the length fixes the optional letter count, every character
    slot is unambiguous, so positional confusion fixing (``0/O``, ``1/I``,
    ``8/B``, ``5/S``, ``2/Z``, ``6/G`` ...) never corrupts letters/digits.
...
class PlateReader:
    ...
    def preprocess(self, plate_bgr: np.ndarray) -> np.ndarray:
        """Grayscale -> CLAHE -> deskew -> upscale, returned as 3-channel."""
        gray = cv2.cvtColor(plate_bgr, cv2.COLOR_BGR2GRAY)
        gray = self._clahe.apply(gray)
        gray = _deskew(gray)
        if gray.shape[1] < 220:
            scale = 220.0 / gray.shape[1]
            gray = cv2.resize(
